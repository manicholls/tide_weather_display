...
def check_mem():
    if gc.mem_free() < 10000:
        print("Memory Low: {0}, restarting".format(gc.mem_free()))
        machine.reset()
    else:
        print("Memory OK: {0}".format(gc.mem_free()))
...
def launcher():
    ...
    graphics.set_pen(1)
    graphics.text("B. Weather", 35, HEIGHT - (265 + y_offset), 600, 3)
...
while True:
    try:
        ...
        ih.led_warn.off()
        gc.collect()
        print("Sleeping {0} minutes".format(ih.app.UPDATE_INTERVAL))
        print(gc.mem_free())
